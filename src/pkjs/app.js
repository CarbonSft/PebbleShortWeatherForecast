    ...
    if (req.readyState === 4) {
      ...
      if (req.status === 200) {
        ...
        if(response !== undefined){
          ...
          var nearestWeather = response.list[0];
          
          console.log("UTC minute offset s:" + utcOffsetSeconds);
          var currentTime = new Date();          
          console.log("Current time is:" + currentTime);
          //var currentTimeUTC = addMinutes(currentTime, utcOffsetSeconds);
          //console.log("Current time in UTC is:" + currentTimeUTC);
          for(var i = 0; i < response.list.length; ++i){
            var forecast = response.list[i];
            var dateFrom = addMinutes(new Date(forecast.dt * 1000), utcOffsetSeconds);
            var val1 = addMinutes(currentTime, forecastPeriodMinutes);
            var val2 = dateFrom;
            console.log("Checking forecast at UTC time:" + forecast.dt);
            console.log("Comparing 1: " + val1);
            console.log("Comparing 2: " + val2);
            if(val1 >= val2 && val1 < addMinutes(val2, 3*60)){
              nearestWeather = forecast;
              console.log("Chose forecast at:" + val2 + " / " + nearestWeather.dt);
              break;
            }
          }
          ...
          var temperature = Math.round(nearestWeather.main.temp - 273.15);
          
          
          //dateFrom.setSeconds(dateFrom.getSeconds() + nearestWeather.dt);
          //console.log(dateFrom);
          console.log(nearestWeather.dt);
          
          
          var rainObject = nearestWeather.rain;
          var rain = 0;
          ...
          var icon = iconFromWeatherId(nearestWeather.weather[0].id);
